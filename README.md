...
- 초기 페이지 배경 이미지 로딩과 동시에 애니메이션이 실행되는 문제

처음 페이지에 방문하면 이미지 캐시가 없는 상태로 배경 이미지가 그려지는 동시에 애니메이션이 진행되어버리는 문제가 있었습니다. (맨 처음 페이지를 방문한다는 가정 하에 진행하기 위해 DevTools의 Network 탭에서 쓰로틀링과 함께 캐시를 비활성화하고 테스트했습니다.) 이러한 문제는 사용자 경험에 부정적인 영향을 미친다고 판단하여 배경 이미지가 로드될 때까지 애니메이션을 지연하는 동작이 필요했습니다.

`Image` 객체를 이용하여 [배경 이미지가 로드될 때까지 대기](src/components/home/SearchArea.vue#L128)했다가 해당 이미지가 로드되면 이미지의 `v-show` 속성을 `true`로 하여 애니메이션이 실행되도록 했습니다. 결과적으로 이미지가 그려지는 모습을 보여주지 않게 되었고, 완전히 로드된 후에야 그려지고 애니메이션이 실행되어 이전보다 시각적 측면에서 부정적 경험을 최소화할 수 있었습니다.
